...
def import_all():
    ...
    # Import dataset
    df = pd.read_csv('scam_data_5.csv')

    # Manipulate imported data for display
    df_to_display = load_data(df)
    
    # Import doc2vec model
    best_model = Doc2Vec.load("doc2vec_model_5.model")

    # Define corpus
    corpus = create_corpus(df, 'preprocessed_text')
    
    # Load the tokeniser
    with open('scam_classifier_augmented_text_tokenizer.pickle', 'rb') as handle:
        tokenizer = pickle.load(handle) 
    
    # Load dictionary for mapping of categories
    a_file = open("scam_type_cat_mapping.pkl", "rb")
